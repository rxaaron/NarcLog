...
if (!$db) {
    ...
    if(isset($_POST['rxnumber'])){
        ...
        exit("Please enter an Rx Number.");
    }
    if(isset($_POST['qtydispensed'])){
        $qtydisp=$_POST['qtydispensed'];
    }else{
        exit("Please enter a dispensed quantity.");
    }
    ...
    $rphchange = $middleonhand - $qtyrem;
        ...
        if ($supres->RPh!=1){
            ...
            $superid=$supres->ID;
            $newtrans=$db->query("INSERT INTO ".$transactiontable." (DateEntered, TransactionDate, TransactionType, Identifier, EmployeeID, Quantity, NewOnHand, DrugID) VALUES (".date("Ymd").",".$dispense.",1,'".$rxnumber."',".$empid.",".$qtydisp.",".$middleonhand.",".$drugid.");");
            if($newtrans){
            ...
            if($updatedrug){
                        exit("<html><head><meta http-equiv=\"Refresh\" content=\"1;url=/narclog/\" /></head><h2>Rx Entry Successful!</h2></html>");
            }
        }
    }
    ...
    if(($oldonhand-$qtydisp)==$qtyrem){
        ...
        $newtrans=$db->query("INSERT INTO ".$transactiontable." (DateEntered, TransactionDate, TransactionType, Identifier, EmployeeID, Quantity, NewOnHand, DrugID) VALUES (".date("Ymd").",".$dispense.",1,'".$rxnumber."',".$empid.",".$qtydisp.",".$qtyrem.",".$drugid.");");
        if($newtrans){

            $updatedrug=$db->query("UPDATE ".$drugtable." SET OnHand=".$qtyrem." WHERE ID=".$drugid.";");
            if($updatedrug){
                        echo "<html><head><meta http-equiv=\"Refresh\" content=\"1;url=/narclog/\" /></head><h2>Rx Entry Successful!</h2></html>";
            }
        }
    }else{
        ...
        echo "<tr><td>Dispense Date:</td><td><input type=\"text\" name=\"dispensedate\" autocomplete=\"off\" value=\"".$dispense."\" /></td></tr>";
        echo "<tr><td>Rx Number:</td><td><input type =\"text\" name=\"rxnumber\" autocomplete=\"off\" value=\"".$rxnumber."\" /></td></tr>";
        echo "<tr><td>Quantity Dispensed:</td><td><input type =\"text\" name=\"qtydispensed\" autocomplete=\"off\" value=\"".$qtydisp."\" /></td></tr>";
